#include <iostream>

using namespace std;

int main()
{
    //Declare variables
    int previous1;
    int previous2;
    int current;
    int counter;
    int nthFibonacci;

    cout << "Enter the first two Fibonacci " << "numbers: "; //Step 1
    cin >> previous1 >> previous2; //Step 2
    cout << endl;

    if ((previous1 <= previous2) && (previous1 >= 0 && previous2 >= 0))
    {
    cout << "The first two Fibonacci numbers are " << previous1 << " and " << previous2 << endl; //Step 3

    cout << "Enter the position of the desired " << "Fibonacci number: "; //Step 4
    cin >> nthFibonacci; //Step 5
    cout << endl;

    if (nthFibonacci == 1) //Step 6.a
    {
        current = previous1;
        cout << "Enter correct value for nth position!" << endl;
    }

    else if (nthFibonacci == 2) //Step 6.b
    {
        current = previous2;
        cout << "Enter correct value for nth position!" << endl;
    }
    else if (nthFibonacci >= 3)//Step 6.c
    {
            counter = 3; //Step 6.c.1
    //Steps 6.c.2 - 6.c.5
    while (counter <= nthFibonacci)
    {
    current = previous2 + previous1; //Step 6.c.2
    previous1 = previous2; //Step 6.c.3
    previous2 = current; //Step 6.c.4
    counter++; //Step 6.c.5
    }//end while
    cout << "The Fibonacci number at position " << nthFibonacci << " is " << current << endl;
    } //Step 7
    else
        cout << "Enter correct value for nth position!" << endl;
    //end else
    }
    else
        {
        cout << "The first number entered must be less than or " << endl;
        cout << "equal to the second number and both the numbers have to be non negative!" << endl;
        }

    return 0;
}//end main
