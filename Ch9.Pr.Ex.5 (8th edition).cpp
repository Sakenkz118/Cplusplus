...
void selection(int Choice[], int Amount[]);
void printCheck(menuItemType MENU[], int Choice[], int Amount[]);
...
int main()
{
    ...
    getData(infile, menu);
    showMenu(menu);
    selection(choice, amount);
    printCheck(menu, choice, amount);
...
void printCheck(menuItemType MENU[], int Choice[], int Amount[])
{
    ...
    for(int index = 0; index < 8; index++)
    {
        if(Choice[index] == 1)
        {
            sum = sum + Amount[index] * MENU[index].menuPrice;
            cout << Amount[index] << " ";
            cout << MENU[index].menuItem;
            cout << right << "   $" << Amount[index] * MENU[index].menuPrice << endl;
        }
    }
    tax = sum * 0.05;
    sum = sum + tax;
